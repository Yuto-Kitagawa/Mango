...
for year in year_list:
    victoria_list.append(
        'https://www.jra.go.jp/datafile/seiseki/g1/victoria/result/victoria' + str(year) + '.html')
...
victoria_html = requests.get(victoria_list[11])
soup = BeautifulSoup(victoria_html.content, "lxml")
umaban_array = []
name_array = []
age_array = []
add_weight_array = []
...
for course in soup.find_all("div", class_='course'):
    ...
    course_detail = course.contents[4]  # コースの詳細
    print(course_dis)
    print(course_detail.text)

print("\n/****************レースの階級****************/")
for grade in soup.find_all("span", class_="grade_icon"):
    grade_content = grade.contents[0].get("alt")
    print(grade_content)
